	...
	module Plugins
		class Pokebot
			...
			def execute(m)
				...
				args = text.split[1..-1]
				handle(m, command, args, m.user)
			end

			def handle(message, command, args, user)
				case command
				when 'newgame'
					new_game(message, user, args[0], args[1], args[2])
				when 'quitgame'
					quit_game(message, user)	
				when 'whosturn'
					whos_turn(message)
				when 'choose'
				...
				when 'getmoves'
					get_moves(user)
				when 'attack'
					attack(message, user, args[0])
				when 'commands'
					commands(user)
				when 'listpokemon'
					list_pokemon(user, args[0])
				else
					no_command(user)
				end
			end
			...
			def new_game(message, user1, nick2, poke_count, default_level)
				...
				if poke_count <= 0 || poke_count > 6
					...
					message.channel.send "#{poke_count} pokemon battle started between #{user1.nick} and #{nick2}, trainers please select your pokemon."
				end
			end

			def quit_game(message, user)
			...
			def choose_pokemon(message, user, *args)
				...
				if !trainer
					...
					if args.any?{|a| a.is_numeric?}
						...
						message.channel.send "#{user.nick} has selected his pokemon."
					end
				else
			...
			def get_moves(user)
				if @battle.get_trainer(user.nick)
					user.notice @battle.get_pokemon(user.nick).list_moves
				else
					user.notice "You are not in the battle."
				end
			end

			def attack(message, user, move)
				if @battle.ready
					return message.channel.send "Both players must choose their pokemon first."
				end
			...
			def commands(user)
				user.notice ["!poke newgame [Opponent_nick] [pokemon_count] [default_level]", "!poke choose [Pokemon_name [Pokemon_level]] [Pokemon_name [Pokemon_level]] etc...",
					"!poke getmoves", "!poke listpokemon [username]", "!poke whosturn", "!poke attack [Move_number]", "!poke quitgame"].join("\n")
			end
			...
			def whos_turn(message)
				message.channel.send "It is #{@battle.turn.nick}'s turn"
			end
		end
	end
end
